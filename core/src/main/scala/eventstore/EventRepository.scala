...
object EventRepository {
  ...
  trait Subscription[EventType] {
    ...
    def stream: ZStream[Scope, Unexpected, EventStoreEvent[EventType]]
  }
  ...
  object Subscription {
    def fromSwitchableStream[EventType](
      ...
      new Subscription[EventType] {
        ...
        override def stream: ZStream[Scope, Unexpected, EventStoreEvent[EventType]] =
...
trait EventRepository[Decoder[_], Encoder[_]] {
  ...
  def getAllEvents[A: Decoder: Tag: DoneBy]
  ...
  def getEventStream[A: Decoder: Tag: DoneBy](
      eventStreamId: EventStreamId
  ): IO[Unexpected, Seq[RepositoryEvent[A]]]

  def saveEvents[A: Decoder: Encoder: Tag: DoneBy](
      eventStreamId: EventStreamId,
      events: Seq[RepositoryWriteEvent[A]]
  ): IO[SaveEventError, Seq[RepositoryEvent[A]]]

  def listen[EventType: Decoder: Tag: DoneBy]: ZIO[Scope, Unexpected, Subscription[EventType]]
